...
| 2026-09-06 | 2.1.260 | `ledger record` on `SubagentStop`; `ledger recall` on `SessionStart` | NOT YET OBSERVED - no subagent has stopped and no new session has started in a project that already had entries since the wiring. The transcript of a new session in `c--repo-lost-mary` or `C--repo-powercountant` should show the "Ledger for" block as injected context; a subagent stop should add a non-`lead-` file. Record here when seen. |
