...
| `scripts/ledger.py record` | `SubagentStop`, no matcher (every role) | Same fields as `check-evidence` (it imports that reader by path): `agent_id`, `agent_type`, `agent_transcript_path` / `transcript_path`, `last_assistant_message`, `stop_hook_active`, plus `cwd` for the ledger location. Observed, not documented: `agent-<id>.meta.json` beside the transcript with `agentType` and `description`; `gitBranch` on records. Always exit 0. | Fields documented; meta/branch shape observed on 2.1.260. Not yet confirmed live. |
| `scripts/ledger.py recall` | `SessionStart`, matcher `startup\|resume\|clear\|compact` | `cwd`; stdout is added to the session's context (documented). Prints nothing when there is no ledger. | Documented (hooks guide: SessionStart stdout -> context; `compact` matcher re-injects after compaction). Not yet confirmed live. |
...
| 2026-09-05 | 2.1.260 | `ledger record` on a real subagent transcript from this machine (review agent, 23 tool uses) with a synthetic payload | OK - entry carried role, description and branch from `.meta.json` / records, the prompt head, and the correct empty edited/ran sets for a read-only role. Live firing on SubagentStop and the SessionStart injection are NOT yet verified: wire both, spawn an `implement`, start a new session, expect the "Ledger for" block. |
