...
| `scripts/witness.py` | `PostToolUse` AND `PostToolUseFailure`, matcher `Bash\|PowerShell\|Edit\|Write\|MultiEdit\|NotebookEdit` | `hook_event_name`, `session_id`, `prompt_id`, `agent_id` and `agent_type` (both absent on the lead's own calls - that absence is what routes a line to `lead.jsonl`), `tool_use_id`, `tool_name`, `tool_input.command` / `.file_path` / `.notebook_path`, `tool_response` (`stdout`, `stderr`, `interrupted`, `filePath`; `exit_code` is read when present and never required - Bash does not send one), and on the failure event `error` (first line `Exit code N`) and `is_interrupt`. Writes one JSON line per call to `<EVIDENCE_ROOT>/<session_id>/agent-<agent_id>.jsonl` or `lead.jsonl` (`EVIDENCE_ROOT` defaults to `~/.claude/agent-library/evidence`), never under `~/.claude/projects`; both ids are matched against `^[A-Za-z0-9_-]{1,64}$` before becoming path parts. Appends in one `write()` on a file opened `ab`, bounds free text to head + tail and the line to 4000 bytes, classifies nothing, prints nothing on stdout, always exits 0. Blind spot, shared with the transcript: a compound command that swallows its exit code (`cmd; echo EXIT=$?`) exits 0 and arrives as `PostToolUse`. | **Confirmed live 2026-09-15 on 2.1.272** with probe hooks - see the log: `PostToolUse` fires inside subagents with a stable `agent_id`, a non-zero exit arrives as `PostToolUseFailure` with no `tool_response`, and Bash success payloads carry no `exit_code` despite the docs. |
...
| 2026-09-15 | 2.1.272 | `check-evidence` reads the witness file instead of the transcript | **NOT YET VERIFIED LIVE - proved by `tests/test-hooks.py` only, which is why it is recorded here as a gap.** What the suite pins, each case failing against the previous version: a transcript holding the edit but not yet the run (the lag this removes) is overruled by a witness file holding both; the witness file wins each disagreement in both directions (transcript failed / witness passed, and the reverse); a `PostToolUseFailure` line blocks a "tests pass" claim; an interrupted call is not a pass; a success with a null `exit_code` is one; a torn line between two valid ones is skipped; a `PostToolUse` edit line is a confirmed edit and a `CHANGED` claim naming it is accepted; the lead's turn is sliced by `prompt_id`, and by the last one in the file when the payload carries none; no witness file means the transcript decides, silently. One case is end to end: the real `witness.py` writes an `Edit`, a passing `pytest -q` and then a `PostToolUseFailure` for the same command, and the real `check-evidence.py` allows the honest report, blocks the same claim once the failure lands, and allows the report of that failure. To verify live: wire both hooks, have an `implement` edit a file and run a check that fails, and expect the bounce while `~/.claude/agent-library/evidence/<session>/agent-<id>.jsonl` holds the lines it quoted. |
