...
| [`scripts/check-spawn.py`](scripts/check-spawn.py) | **PreToolUse hook on `Agent`.** An `implement` spawn whose brief lacks `OWNED`, `OFF-LIMITS`, `DONE MEANS` or `VALIDATION` is blocked before it starts, with the `MISSING:` line - the contract the subagent used to enforce after a full round trip. Other roles are never blocked. It also moves spawns off Fable when Fable is out: once a spawn has failed on the Fable usage or spend limit, every spawn that would run on fable (`explore`, `review`) runs on `opus`, the newest Opus, for the next 6 hours. After that one spawn tries Fable again while the others stay on Opus. The spawns already running when the limit first hits still fail once. Claude Code's own `fallbackModel` never covers a usage limit. And it drops `name` and `team_name` from an `explore`, `review` or `implement` spawn: a named spawn becomes a teammate that stays listed as running after its report and hands the report back truncated. |
...
| [`scripts/guard-shared-checkouts.py`](scripts/guard-shared-checkouts.py) | **PreToolUse hook, config-driven.** In a checkout several sessions use at once it blocks `git switch`, a branch-changing `checkout`, `stash` (but not `stash list`/`show`), `reset --hard|--merge|--keep`, `rebase`, `pull --rebase`, `clean -f`, `gh pr checkout` and `gh pr merge -d`, and points at `git worktree add <dir>_wt_<name> -b <branch>`; in a frozen tree only a deploy script may change, it blocks every git command and every `Edit`/`Write`. Tracks `cd`/`Set-Location`/`git -C` through the command, relative paths included. Without `~/.claude/agent-library/shared-checkouts.json` (template: [`shared-checkouts.example.json`](shared-checkouts.example.json)) it does nothing - except that, config or not, it holds back a merge auto mode would refuse as [Merge Without Review] and then latch on: one with no `review` spawn since the session's last merge of another PR, one not alone in its call (anything beside it but a `cd`, output trimming, `gh pr view`/`checks`/`comment` or a git read or sync holds it; so does a loop), and one through the API (REST or GraphQL). `python scripts/replay-merges.py` replays every merge in the local transcripts through it. |
| [`scripts/prune-worktrees.py`](scripts/prune-worktrees.py) | **CLI, not a hook.** The guard sends sessions to sibling worktrees; this removes the ones that are provably done. It reads the guard's `shared` list, or repositories given as arguments. A worktree is removable only when all of these hold. It has no uncommitted or untracked changes, whatever your git config hides. It has no ignored files except rebuildable caches such as `.venv` and `__pycache__`, because `git worktree remove` deletes ignored files like a `.env` or a runner's sqlite state. Nothing is unpushed. It is merged: a merged PR whose head is its HEAD, or HEAD already in the default branch after at least one commit of its own. And neither its files nor git's own records for it changed within `--idle-hours` (default 24). Everything else is kept with its reason: dirty, ignored file, unpushed, not merged, open PR, no commits of its own yet, active, frozen or locked. Dry run by default; `--apply` runs `git worktree remove` without `--force` and keeps every branch. It also clears empty `<repo>_wt_*` folders left by a remove that a Windows file lock interrupted, and never runs `git worktree prune`. It exits 1 when a repository was skipped or a removal failed. The PR lookup uses the active `gh` account, and a repository that account cannot see falls back to the ancestry test, which only keeps more. |
...
├── scripts/      pycheck.py  check-evidence.py  no-ask.py  no-punt.py  friction.py  ledger.py  check-spawn.py  permit.py  witness.py  guard-shared-checkouts.py  prune-worktrees.py  usage.py  replay-merges.py
├── tests/        test-hooks.py  test-agents.py  test-skills.py  test-prune-worktrees.py  fixtures/pycheck/
