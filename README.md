...
| [`scripts/witness.py`](scripts/witness.py) | **PostToolUse + PostToolUseFailure hooks.** Appends one JSON line per tool call - which agent, which command or file, the exit code, the output (head and tail) - to `~/.claude/agent-library/evidence/<session>/`, so evidence comes from the runtime handing over each call rather than from parsing a transcript the docs call internal, version-unstable and written asynchronously. It classifies nothing: what counts as a validation run is the reader's judgment, not the record's. Fails open on anything unreadable; session directories older than `EVIDENCE_KEEP_DAYS` (default 14) are pruned. |
