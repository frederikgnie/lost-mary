...
| [`scripts/ledger.py`](scripts/ledger.py) | **SubagentStop + SessionStart hooks.** `record` writes one entry per subagent stop to `~/.claude/agent-library/ledger/<project>/` - files it edited (confirmed by the tool result), validation commands and how they exited, what it was asked, its own claims, and any edited file its report does not name - read from its transcript, not from its report. `recall` prints the last entries when a session starts, resumes or is compacted, so the lead never again loses track of who changed what. Kept outside the work tree so nothing a repository ships can pose as the record. |
...
subagent stop - what it edited and ran, from its transcript - and each new,
