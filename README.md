...
| [`scripts/prune-worktrees.py`](scripts/prune-worktrees.py) | **CLI, not a hook.** The guard sends sessions to sibling worktrees; this removes the ones that are provably done. It reads the guard's `shared` list, or repositories given as arguments. A worktree is removable only when it has no uncommitted changes, nothing unpushed, a merged PR whose head is its HEAD (or HEAD already in the default branch) and no file touched within `--idle-hours` (default 6). Everything else is kept with its reason: dirty, unpushed, open PR, active, frozen or locked. Dry run by default; `--apply` runs `git worktree remove` without `--force` and keeps every branch. It also clears empty `<repo>_*` folders left by a remove that a Windows file lock interrupted. The PR lookup uses the active `gh` account, and a repository that account cannot see falls back to the ancestry test, which only keeps more. |
