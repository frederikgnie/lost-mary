...
| [`scripts/ledger.py`](scripts/ledger.py) | **SubagentStop + SessionStart hooks.** `record` appends to `<repo>/.claude/ledger.md` what each subagent *actually* did - files edited, validation commands and how they exited, what it was asked, the head of its report, and any edited file the report does not name - read from its transcript, not from its claims. `recall` prints the last entries when a session starts, resumes or is compacted, so the lead never again loses track of who changed what. |
...
├── scripts/      pycheck.py  check-evidence.py  no-ask.py  no-punt.py  friction.py  ledger.py
...
**Remembering it.** `.claude/ledger.md` (generated, git-ignored) is the record
of what every subagent edited and ran in this repository, and each new,
resumed or compacted session opens with its tail. When a report and the ledger
